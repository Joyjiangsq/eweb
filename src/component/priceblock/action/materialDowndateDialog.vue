<template>
    <div>
        <!--降级选择类型对话框-->
        <dialog :flag.sync="showTag" title="选择降级项" @dialogclick="dialogClickHandler">
...
export default {
  ...
  data: function () {
    return {
      ...
      headercaption:[{type:"radio"},{name:"降级前材料分类", labelValue:"before_code", type:"data"},{name:"降级前分类名称", labelValue:"before_name", type:"data"},
                  {name:"降级后类别", labelValue:"after_code", type:"data"},{name:"降级后分类名称", labelValue:"after_name", type:"data"},
      ...
      dialogClickHandler: function(d) {
          if(d.action == "confirm") {
              let neDate = {
                  ...
                  lv_contact_name: this.curData.after_name,
